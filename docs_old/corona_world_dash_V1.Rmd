...
BrPop <-RawBrPop %>% 
  select(`Unidade da Federação (Código)`, Valor)

BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "11"] <- "RO"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "12"] <- "AC"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "13"] <- "AM"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "14"] <- "RR"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "15"] <- "PA"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "16"] <- "AP"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "17"] <- "TO"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "21"] <- "MA"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "22"] <- "PI"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "23"] <- "CE"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "24"] <- "RN"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "25"] <- "PB"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "26"] <- "PE"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "27"] <- "AL"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "28"] <- "SE"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "29"] <- "BA"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "31"] <- "MG"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "32"] <- "ES"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "33"] <- "RJ"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "35"] <- "SP"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "41"] <- "PR"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "42"] <- "SC"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "43"] <- "RS"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "50"] <- "MS"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "51"] <- "MT"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "52"] <- "GO"
BrPop$`Unidade da Federação (Código)`[BrPop$`Unidade da Federação (Código)` == "53"] <- "DF"
...
# 
# 
# BrCasesLb <-BrCases[-c(28, 29), ]  %>% arrange()
# 
# 
# BrCasesPop <- bind_cols(BrCasesLb[1], map_dfc(BrCasesLb[-1], `/`, BrPop[-1]))
...
# US
USDates <- seq.Date(from       = as.Date('2020-01-22'),
                    to         = Sys.Date() - 1,
                    by         = 'day')
...
BrCasesXts  <- xts(log10(t(BrCases)  + 1), BrDates)
BrDeathsXts <- xts(log10(t(BrDeaths) + 1), BrDates)
...
ects  <- 10**(BrCasesXts) - 1
d0ts  <- ects * apply(ects, 2, function(x) x >= 100)
ld0ts <- lapply(states, function(n) as.vector(coredata(d0ts[,n])))
ld0ts <- lapply(ld0ts, function(x) x[x!=0])
names(ld0ts) <- states
...
# 
# 
# BrMapData <- BrCasesFormatted[states, ] %>% select(last_col(0:0))
# BrMapData <- cbind(states,BrMapData)
# 
# # Get de map
# br_states <- get_brmap(geo = c("State"),
#                             geo.filter = NULL,
#                             class = "SpatialPolygonsDataFrame"
#                             )
# 
# br_states <- join_data(br_states, states, by = c("State" = "states"))
# 
# 
# 
# # Plot
# plot_brmap(br_states)
# 
# 
...
WorldCases %>% 
  ...
  mutate(Dates = as.Date(xtsDates)) %>% 
  filter(Dates >= as.Date("2020-03-02"))



p <- plot_ly(da, x = ~Dates, height = 550)
for (country in countries) {
    countryExpr <- paste0("~","`", country, "`")
    p <- p %>% add_trace(data = da, y = as.formula(countryExpr), name = country, type = 'scatter', mode = 'lines+markers')
}
...
### <b style = "color: #4c1112">  Time to double cases2 </b>
```{r, fig.height = 6, fig.width = 10}
...
g <- ggplot(da, aes(x = Dates))
for (country in countries) {
  g <- g + geom_line(aes_(y = as.name(country) , col = country))
  g <- g + geom_point(aes_(y = as.name(country), col = country))
}
g <- g + scale_colour_manual("", values = plotlyColors[1:length(countries)], breaks = countries)
g <- g + labs(title = "Doubling time", x = "Dates", y = "Doubling time in days") + theme(legend.position = "bottom") 
g
...
<!-- teste <- cbind( WorldCases[1],round( WorldCases[-1]/dife_c,1)) -->
<!-- country <- c("China") -->
<!-- time = seq(0, 181) -->

<!-- ggplot(teste, aes(x=time, y = country)) + geom_line() -->

<!-- teste2<-t(WorldCases[,-1] / dife_c) -->
...
<!-- ggplot() +  -->
<!--   geom_line(data = teste, aes(x = xtsDates, y = "Sweden") -->
...
<!-- ggplot(da) + -->
<!--   geom_line(aes(x = Dates, y = Sweden), color = plotlyColors[1])   -->
...
<!-- dp <- cbind(time, t(WorldCases[,-1] / dife_c)) -->
<!-- colnames(dp)[1] <- "Dias" -->
<!-- dp<-data.frame(dp) -->

<!-- ggplot(dp, aes(x=Dias, y =  Sweden)) + geom_line() -->
